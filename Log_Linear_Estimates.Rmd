  ...
  for (country1 in valid_countries) {
    for (country2 in valid_countries) {
      if (country1 != country2) {
        data <- data.frame(
          Age = Age_Group,
          Male = apply(res_jags_original_simplified$BUGSoutput$sims.list$m[, year, country1, country2, , 2], 2, median),
          Female = apply(res_jags_original_simplified$BUGSoutput$sims.list$m[, year, country1, country2, , 1], 2, median),
          Male_sd = apply(res_jags_original_simplified$BUGSoutput$sims.list$m[, year, country1, country2, , 2], 2, sd),
          Female_sd = apply(res_jags_original_simplified$BUGSoutput$sims.list$m[, year, country1, country2, , 1], 2,sd)
        )
        data_long <- data %>%
